...
function set_frame(frame) {
    ...
    if (ext === 'webm') {
        document.getElementById('frameBody').innerHTML = `<video autoplay muted loop id="video" src="../../frames/${frame}">`
    }
...
function randomFrame(album) {
    console.log(`randframe ${album}`)
    fs.readdir(`frames/${album}/`, function(err, items) {
        newFrame = items[Math.floor(Math.random() * items.length)]
        console.log(newFrame)
        set_frame(`${album}/${newFrame}`);
    });
}

set_frame('default_album/default.webm')
